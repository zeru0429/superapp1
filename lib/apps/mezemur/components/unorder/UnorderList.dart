...
class UnorderList extends StatefulWidget {
  const UnorderList(
  ...
  @override
  _UnorderListState createState() => _UnorderListState();
}
...
class _UnorderListState extends State<UnorderList> {
  ...
  Future<void> _fetchDataFromDatabase() async {
    ...
    try {
      final data = await dbHelper.listData();
...
class Item extends StatelessWidget {
  ...
  Widget _buildChild(BuildContext context, reorderable.ReorderableItemState state) {
    final fontSize = MediaQuery.of(context).size.width * 0.035; // Calculate font size as 3% of screen width
    ...
    final decoration = state == reorderable.ReorderableItemState.dragProxy ||
        ...
        : BoxDecoration(
            border: Border(
              top: isFirst && state != reorderable.ReorderableItemState.placeholder
                  ? Divider.createBorderSide(context)
                  : BorderSide.none,
              bottom: isLast && state == reorderable.ReorderableItemState.placeholder
                  ? BorderSide.none
                  : Divider.createBorderSide(context),
            ),
            color: state == reorderable.ReorderableItemState.placeholder ? null : const Color(0xFFCCE6E9),
          );
    ...
    Widget dragHandle = draggingMode == DraggingMode.iOS
        ? reorderable.ReorderableListener(
            child: Container(
              ...
              child: const Center(
                child: Icon(Icons.reorder, color: Color.fromRGBO(26, 144, 157, 1.0)),
              ),
            ),
          )
        : Container();
    ...
    Widget content = Container(
      ...
      child: SafeArea(
        ...
        bottom: false,
        child: Opacity(
          opacity: state == reorderable.ReorderableItemState.placeholder ? 0.0 : 1.0,
                ...
                Expanded(
                  child: Padding(
                    ...
                    child: Text(
                      data.title,
                      style: TextStyle(
                        fontFamily: "jiret", // Replace with the actual custom font name
                        fontSize: fontSize,
